...
Status: DeepVol-38 adds DEEPVOL_SHARED_TRADING_STATE_MACHINE.md and extracts verified execution state into `packages/deepvol-trading-react/` for both `apps/deepvol-web/` and `apps/deepvol-open-design/`. DeepVol-37 verified-app route handoff remains a secondary fallback. DeepVol-34 adds DEEPVOL_OLD_UI_TRADING_STATE_MACHINE.md and DeepVol-33 adds DEEPVOL_MINTABILITY_RUNTIME_DIAGNOSTICS.md as historical parity context now consumed through the shared-machine boundary. DeepVol-28 adds the standalone `apps/deepvol-open-design/` frontend rewrite with visual-fidelity design system from HTML mockups. DeepVol-25 adds DEEPVOL_PREDICT_MANAGER_UX.md and DEEPVOL_PORTFOLIO_PRIMITIVE_POSITIONS.md for wallet-scoped manager session UX and primitive Portfolio readback. DeepVol-24-fix adds DEEPVOL_RANGE_MINTABILITY_DIAGNOSTICS.md for RANGE candidate failure diagnostics. DeepVol-23 records UP/DOWN primitive Testnet validation success, adds initial RANGE execution path, and adds DEEPVOL_PRIMITIVE_UP_DOWN_VALIDATION.md and DEEPVOL_RANGE_PRIMITIVE_TRADING.md. DeepVol-21 adds mintable strike candidate search for UP/DOWN primitives and DEEPVOL_PRIMITIVE_DIRECT_TRADING.md. Updated for DeepVol-20 fresh BTC MOVE buy validation, DeepVol-18-fix-2 mintable BTC MOVE range validation, active BTC primitive market discovery, and wallet-gated UP/DOWN primitive execution.
...
| [DEEPVOL_PRIMITIVE_EXECUTION_POLICY.md](./DEEPVOL_PRIMITIVE_EXECUTION_POLICY.md) | Primitive execution policy background | Understanding primitive wallet gates, fee policy, and primitive portfolio boundaries |
| [DEEPVOL_PREDICT_PRIMITIVES_FRONTEND.md](./DEEPVOL_PREDICT_PRIMITIVES_FRONTEND.md) | Primitives frontend background | Explaining UP / DOWN / RANGE / MOVE UX relationships and the pre-shared-machine primitive UX boundary |
...
| [DEMO_VIDEO_PLAN.md](./DEMO_VIDEO_PLAN.md) | Scene-by-scene 3–5 minute DeepVol demo recording plan, checklist, and screen-recording tips | Demo / product |
| [DEMO_SCRIPT.md](./DEMO_SCRIPT.md) | English hackathon/demo-day narration for the DeepVol product walkthrough | Demo / product |
...
| [DEEPVOL_PRIMITIVE_EXECUTION_POLICY.md](./DEEPVOL_PRIMITIVE_EXECUTION_POLICY.md) | Documents primitive execution gates, fee policy, and primitive portfolio boundaries | Product / frontend / SDK |
| [DEEPVOL_PREDICT_PRIMITIVES_FRONTEND.md](./DEEPVOL_PREDICT_PRIMITIVES_FRONTEND.md) | Documents UP / DOWN / RANGE / MOVE UX relationships and the pre-shared-machine primitive frontend boundary | Product / frontend |
...
| [DEEPVOL_RANGE_PRIMITIVE_TRADING.md](./DEEPVOL_RANGE_PRIMITIVE_TRADING.md) | Documents DeepVol-23 RANGE primitive execution path: mintable interval search, execution gates, wallet execution, local records, and naming convention | Frontend / SDK / protocol |
...
| [DEEPVOL_MINTABILITY_RUNTIME_DIAGNOSTICS.md](./DEEPVOL_MINTABILITY_RUNTIME_DIAGNOSTICS.md) | Preserves historical DeepVol-33 Open Design execution-parity diagnostics for runtime input normalization, shared candidate diagnostics, DUSDC balance separation, and connected-wallet acceptance; active Open Design diagnostics should render shared `machine.diagnostics` only | Frontend / SDK / protocol / diagnostics |
| [DEEPVOL_OLD_UI_TRADING_STATE_MACHINE.md](./DEEPVOL_OLD_UI_TRADING_STATE_MACHINE.md) | Preserves historical DeepVol-34 old verified UI trading sequence for MOVE, UP/DOWN, and RANGE; DeepVol-38 implements the active shared state layer in `packages/deepvol-trading-react` | Frontend / SDK / state machine |
| [DEEPVOL_SHARED_TRADING_STATE_MACHINE.md](./DEEPVOL_SHARED_TRADING_STATE_MACHINE.md) | Documents `@rangepilot/deepvol-trading-react`, exported MOVE / UP / DOWN / RANGE machines, app boundaries, safety invariants, fallback routing, and no-real-transaction verification rules | Frontend / SDK / state machine |
...
| [DEEPVOL_FRONTEND_MVP.md](./DEEPVOL_FRONTEND_MVP.md) | Defines `apps/deepvol-web/` as the verified executable wallet-gated DeepVol app and records the shared-machine package boundary, wallet prompts, PredictManager setup/funding, quote, preflight, VolSeries creation, buy/mint/redeem/deposit/withdraw controls, and validation history | Frontend / product / SDK |
| [DEEPVOL_OPEN_DESIGN_REWRITE_APP.md](./DEEPVOL_OPEN_DESIGN_REWRITE_APP.md) | Documents `apps/deepvol-open-design/` as the standalone Open Design view layer over shared machines with visual-fidelity design system, product/status/portfolio readback, `VITE_DEEPVOL_VERIFIED_APP_URL`, and verified fallback route mapping | Frontend / product / design |
...
6. [DEEPVOL_SHARED_TRADING_STATE_MACHINE.md](./DEEPVOL_SHARED_TRADING_STATE_MACHINE.md)
7. [DEEPVOL_PRIMITIVE_EXECUTION_POLICY.md](./DEEPVOL_PRIMITIVE_EXECUTION_POLICY.md)
...
|---|---|
| DeepVol product | DeepVol product direction, primitives and receipts, MVP scope, primitive execution policy, Predict primitives frontend, primitive quote/preflight, shared trading state machine, frontend MVP, browser buy validation, browser redeem validation, redeem and settlement flow, redeem preflight validation, protocol architecture, data model, business model, ADR-0003 |
...
| DeepVol Predict primitives frontend | DeepVol product direction, primitives and receipts, primitive execution policy, Predict primitives frontend, primitive quote/preflight, shared trading state machine, PredictManager UX, primitive Portfolio positions, primitive active market discovery, RANGE mintability diagnostics, mintability runtime diagnostics, old UI trading state machine, MVP scope, frontend MVP, protocol integration notes, entrypoint bindings plan |
