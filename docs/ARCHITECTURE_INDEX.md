...
| [DEEPVOL_MINTABILITY_RUNTIME_DIAGNOSTICS.md](./DEEPVOL_MINTABILITY_RUNTIME_DIAGNOSTICS.md) | Preserves historical DeepVol-33 Open Design execution-parity diagnostics for runtime input normalization, shared candidate diagnostics, DUSDC balance separation, and connected-wallet acceptance; active Open Design behavior is presentation/handoff only | Frontend / SDK / protocol / diagnostics |
| [DEEPVOL_OLD_UI_TRADING_STATE_MACHINE.md](./DEEPVOL_OLD_UI_TRADING_STATE_MACHINE.md) | Preserves historical DeepVol-34 old verified UI trading state machine and Open Design parity target for MOVE, UP/DOWN, and RANGE; active Open Design behavior is presentation/handoff only | Frontend / SDK / state machine |
...
| [DEEPVOL_FRONTEND_MVP.md](./DEEPVOL_FRONTEND_MVP.md) | Defines `apps/deepvol-web/` as the verified executable wallet-gated DeepVol app for trading state machines, wallet prompts, PredictManager setup/funding, quote, preflight, VolSeries creation, buy/mint/redeem/deposit/withdraw controls, and validation history | Frontend / product / SDK |
| [DEEPVOL_OPEN_DESIGN_REWRITE_APP.md](./DEEPVOL_OPEN_DESIGN_REWRITE_APP.md) | Documents `apps/deepvol-open-design/` as the standalone Open Design presentation and verified-app handoff surface with visual-fidelity design system, product/status/portfolio readback, `VITE_DEEPVOL_VERIFIED_APP_URL`, and verified route mapping | Frontend / product / design |
