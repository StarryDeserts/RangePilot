...
| 4 | `predict_manager::range_position` | Read active quantity for a specific range key | Phase 1D-1 direct devInspect read returned `1000` twice for the first minted range |
| 5 | `range_key::new` | Construct the range key for lower/upper strikes | Phase 1C-fix scanner builds spot/forward-centered ranges and successful quote devInspect returns; Phase 1D-1 used it for direct position readback |
| 6 | `predict::get_range_trade_amounts` | Preview official range trade amounts | Phase 1C-fix2 devInspect verified safe `(mint_cost, redeem_payout)` decoding and found positive official range quotes |
...
| 8 | `predict::redeem_range<DUSDC>` | Redeem or claim range position through official protocol path | Confirmed entrypoint role; live vs settled behavior must be validated |
...
| Sui events/checkpoints | History, portfolio fallback, creator stats, oracle freshness stream | `RangeMinted` event readback and normalized key fields verified for known digest; broader event query strategy remains pending |
| Direct object reads | Wallet-critical state: Predict Account balance, positions, claimability | Direct `range_position` devInspect verified for a specific RangeKey; balance and position enumeration remain pending |
