...
Local source snapshot used for wrapper build/debug: `deepbookv3-predict-testnet-4-16/packages/predict`. The full local third-party snapshot root `deepbookv3-predict-testnet-4-16/` is ignored and must not be committed. Official Sui docs and `DEEPBOOK_PREDICT_OFFICIAL_CONTRACT_INFO.md` remain the deployment/config source of truth. The wrapper package is unpublished, package ID is `TBD`, and client builders must block unless an explicit wrapper package ID is supplied.

Build status: `npm run move:build:rangepilot` passes against `deepbookv3-predict-testnet-4-16/packages/predict`, and `npm run move:test:rangepilot` passes with 2 RangePilot fee-helper tests. The old incomplete `deepbookv3-predict-package/predict` dependency blocker is superseded.
