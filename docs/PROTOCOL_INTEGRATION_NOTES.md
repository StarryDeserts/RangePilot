...
## DeepVol-3 local package status

DeepVol-3 adds a local-only `move/deepvol` package, TypeScript types/config/SDK stubs, and documentation for `VolSeries` plus non-custodial `MoveReceipt`. The DeepVol Testnet config intentionally keeps `packageId = null` and `protocolVaultId = null` until the user manually reviews publish configuration, publishes the package, and provides real deployment values. No DeepVol package was published, no real `VolSeries` was created, no real `MoveReceipt` was minted, no ProtocolVault deposit was made, and no transaction was submitted in this phase.
