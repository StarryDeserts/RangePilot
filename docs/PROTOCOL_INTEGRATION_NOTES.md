...
For DeepVol direction and binary-leg requirements, see [DEEPVOL_PRODUCT_DIRECTION.md](./DEEPVOL_PRODUCT_DIRECTION.md), [DEEPVOL_PRIMITIVES_AND_RECEIPTS.md](./DEEPVOL_PRIMITIVES_AND_RECEIPTS.md), [DEEPVOL_MVP_SCOPE.md](./DEEPVOL_MVP_SCOPE.md), [DEEPVOL_PROTOCOL_ARCHITECTURE.md](./DEEPVOL_PROTOCOL_ARCHITECTURE.md), [DEEPVOL_MOVE_RECEIPT_CONTRACT.md](./DEEPVOL_MOVE_RECEIPT_CONTRACT.md), [DEEPVOL_CONTRACT_BUILD_VALIDATION.md](./DEEPVOL_CONTRACT_BUILD_VALIDATION.md), [DEEPVOL_BINARY_LEG_INTEGRATION.md](./DEEPVOL_BINARY_LEG_INTEGRATION.md), [DEEPVOL_BINARY_MINT_TESTNET_VALIDATION.md](./DEEPVOL_BINARY_MINT_TESTNET_VALIDATION.md), and [ADR/0003-pivot-to-deepvol-btc-move.md](./ADR/0003-pivot-to-deepvol-btc-move.md).
...
## DeepVol-3B local Route B package status

DeepVol-3B adds a local-only `move/deepvol` Route B package, TypeScript types/config/SDK stubs, and documentation for `VolSeries`, DeepVol-owned `ProtocolVault<Quote>`, and non-custodial but protocol-enforced `MoveReceipt`. The public receipt path is `receipt::buy_move_receipt<Quote>`: it derives UP/DOWN legs from `VolSeries`, quotes both legs, internally calls DeepBook Predict `predict::mint<Quote>` twice, deposits Create Fee into DeepVol `ProtocolVault<Quote>`, and only then mints the receipt. The DeepVol Testnet config intentionally keeps `packageId = null`, `protocolVaultId = null`, and `adminCapId = null` until the user manually reviews publish configuration, publishes the package, creates the quote-asset vault, and provides real deployment values. No DeepVol package was published, no real `VolSeries` was created, no real `MoveReceipt` was minted, no DeepVol Route B transaction was executed, no ProtocolVault deposit was made, and no transaction was submitted in this phase.
