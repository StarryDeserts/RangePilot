...
Status: Updated with confirmed official DeepBook Predict Testnet values and DeepVol-14 primitive quote/preflight preview policy.
...
The current BTC MOVE route can show quote/readback state but must keep the buy button disabled when browser `buy_move_receipt<DUSDC>` preflight is unavailable or stale for the current quote and wallet state. The `/primitives` route can run browser-safe UP/DOWN/RANGE quote and mint-preflight diagnostics for the configured BTC MOVE series, but direct primitive wallet execution remains disabled even when quote and preflight pass. Direct primitive previews do not create `MoveReceipt` objects and do not charge DeepVol Create Fee. General portfolio enumeration/indexer support is not available; localStorage receipts, labeled validation receipts, and known-key primitive readback are reference/readback surfaces only.
