...
Status: DeepVol-24-fix adds RANGE mintability diagnostics while preserving the RANGE wallet gate. DeepVol-23: UP/DOWN primitive direct mint validated on Testnet; RANGE execution path added (pending validation); see DEEPVOL_PRIMITIVE_UP_DOWN_VALIDATION.md and DEEPVOL_RANGE_PRIMITIVE_TRADING.md. DeepVol-21 adds mintable strike candidate search for UP/DOWN primitives; execution gate now requires primitiveMintabilityStatus passed. Updated for DeepVol-20: fresh BTC MOVE buy validated on Testnet with active VolSeries. BuyMovePage detects stale/missing/validation-required/non-mintable VolSeries, provides Regenerate mintable range, and gates Create BTC MOVE Series plus buy behind mintability validation. See DEEPVOL_ACTIVE_MOVE_SERIES.md and DEEPVOL_MINTABLE_MOVE_RANGE.md.
