...
Status: DeepVol-24-fix adds RANGE mintability diagnostics while preserving the RANGE wallet gate. DeepVol-23 records UP/DOWN primitive Testnet validation success and adds RANGE execution path (pending validation). DeepVol-21 adds mintable strike candidate search and execution gates for UP/DOWN primitives. Updated for DeepVol-20: fresh BTC MOVE buy validated on Testnet with active VolSeries. BTC MOVE buy no longer falls back to historical configured VolSeries, and Create BTC MOVE Series plus quote/preflight/wallet review are gated on mintable range validation.
