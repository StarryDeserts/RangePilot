...
Status: Updated for DeepVol-11 guided redeem source confirmation, read/preflight validation, and Portfolio scaffold.
...
| Deliverables | Landing narrative; DeepVol-7 polished `apps/deepvol-web` BTC MOVE market, buy, and portfolio screens; completed browser-safe receipt creation preflight; DeepVol-10 browser buy validation artifact; portfolio receipt display evidence; DeepVol-11 guided redeem read/preflight artifact and Portfolio scaffold; demo script; fallback screenshots; runbook. |
| Non-goals | Broad market support, pro API, production-grade indexer, custodial receipt, secondary market, Profit Fee enforcement, redeem execution before DeepVol-12 approval. |
| Acceptance criteria | Demo explains “Trade movement, not direction”; live or recorded evidence shows official DeepBook Predict binary composition, receipt metadata, browser wallet buy validation, portfolio display, and disabled guided redeem preflight; limitations around non-custodial receipt, local/indexer-limited portfolio state, and pending real redeem execution are not misrepresented. |
...
DeepVol-12 should perform controlled browser redeem validation only after explicit approval: use the known wallet/receipt, run read checks, run explicit preflight, confirm payout estimate and manager balance before prompt, execute one controlled Testnet redeem through wallet approval, parse `PositionRedeemed`, reconcile manager DUSDC balance and position deltas, and only then update local receipt status.
