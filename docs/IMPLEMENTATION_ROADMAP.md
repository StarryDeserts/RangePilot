...
Status: Updated for DeepVol-14 primitive quote/preflight previews.
...
| Acceptance criteria | Users can connect a Testnet wallet, inspect the validated BTC MOVE VolSeries, understand `Trade movement, not direction.`, create or store a PredictManager, inspect wallet DUSDC funding, refresh quotes, run browser `buy_move_receipt<DUSDC>` preflight, understand why the buy button is disabled until receipt preflight and funding gates pass, execute `buy_move_receipt<DUSDC>` by explicit wallet approval after gates pass, view the browser-created receipt in portfolio without treating the receipt as custody truth, run explicit guided redeem preflight that reads UP/DOWN positions and payout estimates, execute the controlled known-receipt redeem only after exact known receipt/wallet/Testnet/fresh preflight gates pass, and understand UP/DOWN/RANGE as preview-only advanced primitives that can quote/preflight diagnostics but do not create `MoveReceipt` or enable direct primitive wallet execution. Current status: DeepVol-10 manually validated browser digest `A6YB62BqMmWsQeEZUoh4qYAA6n4RMqnih5TtHRdadfGn`, receipt `0xbbc2d18447502830a96602b8f9611e834c509d6fa00abdf2061ecd1addaa35eb`, actual premium `9973`, Create Fee `29`, and portfolio display; DeepVol-11 read/preflight validation observed current manager-level UP/DOWN quantities of `20000`, selected receipt quantity `10000`, and devInspect redeem preflight passed for both receipt-scoped legs; DeepVol-13 validated browser redeem digest `HeHNeZ95oymZzmA2ZpdjkvJgCaA9s5DzL7qs6aCgbJbJ`, UP payout `9727`, DOWN payout `47`, total payout `9774`, and UP/DOWN position deltas `20000 -> 10000`. |
...
| Deliverables | Landing narrative; DeepVol-7 polished `apps/deepvol-web` BTC MOVE market, buy, and portfolio screens; completed browser-safe receipt creation preflight; DeepVol-10 browser buy validation artifact; portfolio receipt display evidence; DeepVol-11 guided redeem read/preflight artifact; DeepVol-13 controlled redeem validation artifact; quote/preflight preview UP / DOWN / RANGE primitive cards and route; demo script; fallback screenshots; runbook. |
...
| Acceptance criteria | Demo explains “Trade movement, not direction”; live or recorded evidence shows official DeepBook Predict binary composition, receipt metadata, browser wallet buy validation, portfolio display, guided redeem preflight, controlled browser redeem validation, and primitive quote/preflight preview education; limitations around non-custodial receipt, local/indexer-limited portfolio state, known-receipt-only redeem validation, known-key-only primitive readback, and disabled primitive execution are not misrepresented. |
