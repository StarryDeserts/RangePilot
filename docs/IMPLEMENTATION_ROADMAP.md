...
- DeepVol-30 adds PredictManager setup CTA to `apps/deepvol-open-design/`: connected-wallet users who lack a PredictManager see an actionable "Create PredictManager" button instead of dead-end blocker text. The create flow uses real `signAndExecuteTransaction`. Manual object ID entry remains Advanced-only. Passive blocker pills removed from BtcMarketPage. It does not change Move contracts or execute real chain transactions.
- DeepVol-31 fixes three P0 connected-wallet UX issues in `apps/deepvol-open-design/`: (1) MoveExecutionPanel shows active market context strip so MOVE doesn't appear broken when VolSeries is idle, (2) PredictManagerSetup shows ManagerFundingCard with DUSDC deposit CTA when manager is ready but unfunded, (3) funding status is visually separated from quote/preflight errors. It does not change Move contracts or execute real chain transactions.
- DeepVol-32 fixes five root causes in Open Design trading chain: (1) shared `primitiveQuoteGate.ts` no longer uses MOVE-specific blocker text for all products, (2) `buildPrimitivePreflightBlockers()` gates on mintability status, (3) MOVE range band falls back to `activeMarket.suggestedLowerStrike`/`suggestedUpperStrike` before "TBD", (4) RANGE preflight blocked when mintability failed prevents `ARITHMETIC_ERROR`, (5) mintability status threaded through `usePrimitivePreflight` params.
