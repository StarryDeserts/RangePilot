...
| Phase 1B: PredictManager create/deposit | Add wallet UI scaffold, DUSDC wallet balance read, local manager hint discovery, `create_manager` PTB scaffold, DUSDC coin selection, and blocked deposit/balance read scaffolds. | Scaffold exists; `deposit<DUSDC>`, manager ID recovery, public server/event discovery, and manager balance read remain recorded blockers before real deposit validation. |
