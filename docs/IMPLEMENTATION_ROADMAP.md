...
Status: Updated for DeepVol-12 controlled browser redeem wiring and current wallet-extension blocker evidence.
...
| Acceptance criteria | Users can connect a Testnet wallet, inspect the validated BTC MOVE VolSeries, understand `Trade movement, not direction.`, create or store a PredictManager, inspect wallet DUSDC funding, refresh quotes, run browser `buy_move_receipt<DUSDC>` preflight, understand why the buy button is disabled until receipt preflight and funding gates pass, execute `buy_move_receipt<DUSDC>` by explicit wallet approval after gates pass, view the browser-created receipt in portfolio without treating the receipt as custody truth, run explicit guided redeem preflight that reads UP/DOWN positions and payout estimates, and see controlled redeem blockers unless the exact known receipt/wallet/Testnet/fresh preflight gates pass. Current status: DeepVol-10 manually validated browser digest `A6YB62BqMmWsQeEZUoh4qYAA6n4RMqnih5TtHRdadfGn`, receipt `0xbbc2d18447502830a96602b8f9611e834c509d6fa00abdf2061ecd1addaa35eb`, actual premium `9973`, Create Fee `29`, and portfolio display; DeepVol-11 read/preflight validation observed current manager-level UP/DOWN quantities of `20000`, selected receipt quantity `10000`, and devInspect redeem preflight passed for both receipt-scoped legs; DeepVol-12 wired controlled browser execution but no real redeem was executed because the validation browser had no approved wallet extension/account available. |
...
| Deliverables | Landing narrative; DeepVol-7 polished `apps/deepvol-web` BTC MOVE market, buy, and portfolio screens; completed browser-safe receipt creation preflight; DeepVol-10 browser buy validation artifact; portfolio receipt display evidence; DeepVol-11 guided redeem read/preflight artifact; DeepVol-12 controlled redeem wiring/blocker artifact; demo script; fallback screenshots; runbook. |
...
| Acceptance criteria | Demo explains “Trade movement, not direction”; live or recorded evidence shows official DeepBook Predict binary composition, receipt metadata, browser wallet buy validation, portfolio display, guided redeem preflight, and controlled redeem blockers; limitations around non-custodial receipt, local/indexer-limited portfolio state, missing-wallet browser blocker, and pending real redeem execution are not misrepresented. |
...
DeepVol-12 wires controlled browser redeem execution for the known receipt and blocks the wallet prompt unless the exact receipt, exact owner wallet, Sui Testnet, both receipt-scoped leg preflights, fresh submit-time read/preflight, and local one-shot guard pass. The validation browser did not have the approved wallet extension/account available, so no real redeem executed and no digest/events/post-state reconciliation exist yet. The next real validation must use an already-installed approved wallet on Sui Testnet, approve one combined two-leg `predict::redeem<DUSDC>` PTB, parse `PositionRedeemed`, reconcile manager DUSDC balance and position deltas, and only then update local receipt status.
