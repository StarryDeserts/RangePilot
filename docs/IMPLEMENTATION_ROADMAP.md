...
| Phase 1D-2: `redeem_range` validation | Confirm active/settled range display, redeem preview if available, and `predict::redeem_range<DUSDC>`. | Not executed. Must preserve direct `range_position` pre/post readback checks and stay Testnet-only. |
